...
if (strlen($_SESSION['aid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $query = mysqli_query($con, "update tblemergencyreport set assignTo='$assignto',assignTme='$assigntime',status='Assigned' where id='$rid'");
        if ($query) {
    ...
    if (isset($_POST['takeaction'])) {
        ...
        $status = $_POST['status'];
        $remark = $_POST['remark'];
        $query = mysqli_query($con, "insert into tblemergencyrequesthistory(requestId,status,remark) values('$rid','$status','$remark')");
        if ($query) {
            $query = mysqli_query($con, "update tblemergencyreport set status='$status' where id='$rid'");
                    ...
                    <div class="modal-body">
                        <form method="post" enctype="multipart/form-data">
                            <p> <select class="form-control" name="status" id="status" required="true">
                                    ...
                                    $query1 = mysqli_query($con, "select status from tblemergencyreport where id='$rid'");
